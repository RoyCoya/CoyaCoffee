    ...
    */
    ...
    var start = function () {
      ...
      for (; start_date.getTime() <= end_date.getTime(); start_date.setDate(start_date.getDate() + 1)) {
        ...
        item_html += '<rect class="day" width="12" height="12" y="' + y + '" fill="' + color + '" data-count="' + count + '" data-date="' + data_date + '" rx="' + radius + '" ry="' + radius + '"/>';
      ...
      for (var i = 0; i < month_position.length; i++) {
        ...
        loop_html += '<text x="' + item.x + '" y="-5" class="month" fill="#eeeeee">' + month_name + '</text>';
      }

      //Add Monday, Wenesday, Friday label
      loop_html += '<text text-anchor="middle" class="wday" fill="#eeeeee" dx="-10" dy="22">{0}</text>'.formatString(settings.h_days[0]) +
        '<text text-anchor="middle" class="wday" dx="-10" fill="#eeeeee" dy="48">{0}</text>'.formatString(settings.h_days[1]) +
        '<text text-anchor="middle" class="wday" dx="-10" fill="#eeeeee" dy="74">{0}</text>'.formatString(settings.h_days[2]);

      //Fixed size for now with width= 721 and height = 110
      var wire_html =
        '<svg width="1200" height="150" viewBox="0 0 770 110"  class="js-calendar-graph-svg">' +
    ...
    var mouseEnter = function (evt) {
      ...
      var text = "{0} {1} 于 {2}".formatString(count, count_text, date);
